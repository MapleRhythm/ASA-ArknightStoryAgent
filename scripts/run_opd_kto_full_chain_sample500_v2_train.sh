...
"$PYTHON_BIN" scripts/build_opd_kto_dataset_from_scores.py \
  ...
  --output-dir data/processed/llama_factory/opd_kto_full_chain_sample500_deepseek_v2 \
  --val-ratio 0.08 \
  --max-evidence-items 3 \
  --max-evidence-chars 360 \
  --negative-ratio-cap 1.8

"$PYTHON_BIN" -m llamafactory.cli train \
  src/config/llama_factory_opd_kto_full_chain_sample500_v2_config.yaml
