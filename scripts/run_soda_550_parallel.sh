...
if [[ ! -f "$BASE_OUT_DIR/audit_records.jsonl" ]]; then
  echo "[error] Missing existing base SODA data: $BASE_OUT_DIR/audit_records.jsonl" >&2
  echo "        This script expects the previous 50-sample rollout to exist there." >&2
  exit 2
fi
...
if [[ "$SKIP_GENERATION" != "1" ]]; then
  run_soda_shard 0 gpu0 167 50 &
  pid0=$!
  run_soda_shard 1 gpu1 167 217 &
  pid1=$!
  run_soda_shard 2 gpu2 166 384 &
  pid2=$!
  ...
  wait "$pid2" || fail=1
  if [[ "$fail" != "0" ]]; then
    echo "[error] At least one SODA shard failed. Check logs/soda_550_gpu*.log" >&2
    exit 1
  fi
else
...
failed = False
for shard in ("gpu0", "gpu1", "gpu2"):
    path = Path(f"data/processed/llama_factory/soda_blackbox_deepseek_v1_shard_{shard}/build_summary.json")
    if not path.exists():
    ...
    print(f"[shard-check] {shard} records={records} student_failed={student_failed}")
