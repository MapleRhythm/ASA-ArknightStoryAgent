...
- answer 必须为空字符串。
- missing_slots：必须是 2-5 个具体的可检索语义缺口短语，例如「太师的真实身份」「烛煌的师承关系」；
...
def build_conclusion_prompt_bundle(
    *,
    ...
    requirements = [
        ...
        "7. user prompt 必须包含：用户问题、多轮上下文、当前假设文档(JSON)、当前证据；当前假设文档不能是空对象 {}。",
        "8. conclusion prompt 必须显式带出当前检索轮次与历史检索上下文。",
        "9. user prompt 中的“当前证据”必须使用证据包中**至少 6 段、最多全部 12 段** evidence；不要只摘 1-2 段；保留 `[证据 N]` 与 clean_text 块结构，让线上输入分布与训练对齐。",
        "10. 若要构造 clarify_user 样本，必须主动在用户问题里写入代词（她/他/这位 等）或同名实体（如\"博士\"、\"戈尔丁\"、\"魔王\"、\"陈\"等），并把多轮上下文留空或刻意只给少量信息使指代无法唯一确定。",
        "11. 若要构造 abstain 样本，必须挑选证据中**核心实体与用户问题完全不重叠**的子集（例如证据是另一活动的片段、或只剩进驻设施语音之类的无关片段），让结论无法成立。",
        "12. answer_directly / abstain 的 answer 文本绝对不允许出现\"根据证据\"\"根据剧情\"\"根据检索\"\"基于证据\"\"从证据中\"\"根据上面\"等暴露检索过程的措辞。",
        "13. answer_directly 时 answer 中出现的所有关键实体名、关系词、地点、事件，都必须能在“当前证据”原文中找到字面对应或显著近义；不允许只在 hypothesis.keywords 中出现却不在证据中的内容。",
        "14. retrieve_more 时 missing_slots 必须列出 2-5 个具体可检索缺口；禁止使用\"更多信息\"\"相关背景\"\"详细资料\"\"完整剧情\"等空泛词。",
        "15. clarify_user 时 clarification_question 必须列出 2-4 个候选解读，例如 \"您指的是 A、B 还是 C？\"；不允许只说\"请澄清\"。",
        "16. abstain 时 answer 第一句必须是 \"现有检索证据不足以确认…\" 模板的变体；必须显式指出**缺哪个关键事实**，禁止笼统说\"不知道\"。",
        "17. 结论生成样本必须显式体现“基于当前证据是否足够作答”的判断；不要随意把 answer_directly 与 retrieve_more 互换。",
        "18. 所有文本使用中文，不要编造英文别名或 Dr. 前缀。",
        "19. 顶层返回格式必须是一个 JSON 对象，且只有 `samples` 字段。",
    ]
    user_prompt = (
